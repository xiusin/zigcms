...
test "MemoryCacheDriver basic operations" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();

    var driver = MemoryCacheDriver.init(allocator);
    defer driver.deinit();

    var cache_interface = driver.asInterface();

    // 测试设置和获取
    try cache_interface.set("test_key", "test_value", null);
    const value = cache_interface.get("test_key");
    try std.testing.expect(value != null);
    try std.testing.expect(std.mem.eql(u8, value.?, "test_value"));

    // 测试存在性
    try std.testing.expect(cache_interface.exists("test_key"));
    try std.testing.expect(!cache_interface.exists("nonexistent"));

    // 测试删除
    try cache_interface.del("test_key");
    try std.testing.expect(!cache_interface.exists("test_key"));
}

test "MemoryCacheDriver stats" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();
    ...
    // 添加一些测试数据
    try cache_interface.set("key1", "value1", null);
    try cache_interface.set("key2", "value2", null);
