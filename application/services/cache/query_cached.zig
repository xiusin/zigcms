    ...
    return struct {
        ...
        pub fn count(self: *Self) !u64 {
            ...
            if (try self.cache_service.get(cache_key)) |cached_count_str| {
                const cached_val = try std.fmt.parseInt(u64, cached_count_str, 10);
                return cached_val;
            }

            // 没有缓存，执行查询
            const result_count = try self.query_builder.count();

            // 存入缓存
            const count_str = try std.fmt.alloc(self.query_builder.allocator, "{}", .{result_count});
        ...
        /// 内部序列化实现
        fn serializeSingleModelInternal(_: *const Self, model: Model, result: *std.ArrayList(u8)) !void {
        ...
        /// 反列化单个模型
        fn deserializeSingleModel(_: *const Self, json_str: []const u8) !Model {
            _ = json_str; // 简化的实现
            // 这里应该解析JSON并创建模型实例
