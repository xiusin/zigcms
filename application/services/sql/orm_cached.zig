...
const CacheService = @import("../../application/services/cache/cache.zig").CacheService;

/// 带有缓存功能的模型定义
pub fn defineCached(comptime T: type) type {
    return struct {
        const Self = @This();
        usingnamespace orm.define(T);
        ...
        pub fn count() !u64 {
            ...
            // 没有缓存，查询数据库
            const count = try Self.CountWithDB(global_db.?);
            
            // 将数量转换为字符串并存入缓存
            const count_str = try std.fmt.alloc(std.heap.page_allocator, "{}", .{count});
        ...
        fn UpdateWhereWithDB(db: *orm.Database, condition: anytype, updates: anytype) !u64 {
            var query = Self.Query(db);
            defer query.deinit();
            return query.whereEq(@fieldNames(@TypeOf(condition))[0], @field(condition, @fieldNames(@TypeOf(condition))[0])).update(updates);
        }
