...
pub const PluginSystemService = struct {
    ...
    pub fn generateReport(self: *PluginSystemService) ![]u8 {
        ...
        for (plugin_names) |name| {
            ...
            var cap_count: u32 = 0;
            if (caps.http_handlers) { try result.writer().print("HTTP,", .{}); cap_count += 1; }
            if (caps.database_drivers) { try result.writer().print("数据库,", .{}); cap_count += 1; }
            if (caps.authentication) { try result.writer().print("认证,", .{}); cap_count += 1; }
            if (caps.middleware) { try result.writer().print("中间件,", .{}); cap_count += 1; }
            if (caps.event_listeners) { try result.writer().print("事件,", .{}); cap_count += 1; }
            if (caps.scheduled_tasks) { try result.writer().print("任务,", .{}); cap_count += 1; }
            if (caps.custom_routes) { try result.writer().print("路由,", .{}); cap_count += 1; }
            if (caps.file_processors) { try result.writer().print("文件,", .{}); cap_count += 1; }
            if (caps.cache_providers) { try result.writer().print("缓存,", .{}); cap_count += 1; }
            if (caps.storage_backends) { try result.writer().print("存储,", .{}); cap_count += 1; }
            if (caps.encryption) { try result.writer().print("加密,", .{}); cap_count += 1; }
            if (caps.logging) { try result.writer().print("日志,", .{}); cap_count += 1; }
            
            if (cap_count == 0) {
                try result.writer().print("无", .{});
            } else {
                try result.writer().print("\b", .{}); // 移除最后的逗号
            }
            try result.writer().print("\n", .{});
            try result.writer().print("\n", .{});
        }
