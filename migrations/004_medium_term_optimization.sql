...
CREATE TABLE IF NOT EXISTS `alert_rules` (
  ...
  `rule_type` VARCHAR(50) NOT NULL COMMENT '规则类型: brute_force, sql_injection, xss, etc.',
...
-- 4. 添加告警规则关联字段到告警表（如果需要）
ALTER TABLE `security_alerts` 
ADD COLUMN `rule_id` INT COMMENT '触发的规则ID' AFTER `alert_type`,
ADD INDEX `idx_rule_id` (`rule_id`);

-- 5. 创建性能指标表（可选，用于持久化性能监控数据）
...
-- 7. 创建 WebSocket 连接表（可选，用于管理 WebSocket 连接）
