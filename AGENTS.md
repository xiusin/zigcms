...
// ✅ 嵌套预加载：一次性加载多层关系
var q = OrmRole.Query();
_ = q.with(&.{"menus.permissions"});  // 加载菜单及其权限
const roles = try q.get();
defer OrmRole.freeModels(roles);
...
2. **SQL 安全**：参数化查询 + 注入防护
3. **架构清晰**：整洁架构 + 依赖倒置
4. **性能优化**：嵌套预加载 + 关系预加载 + 批量查询 + 缓存策略
5. **代码质量**：显式错误处理 + 资源管理
5. **实现前提**：确认无相同功能或类似已实现
