...
const dict_service = @import("../../application/services/cache/dict_service.zig");
...
/// 删除字典项
pub fn deleteDict(self: *Self, req: zap.Request) !void {
    req.parseQuery();
    ...
    const affected = try dict_service.deleteDict(id);
    if (affected == 0) {
...
/// 统计字典项数量
pub fn countDict(self: *Self, req: zap.Request) !void {
    req.parseQuery();
    ...
    const count = try dict_service.countDictByType(dict_type_param);
    
    base.send_ok(req, .{ .count = count });
}

/// 获取字典项标签
pub fn getDictLabel(self: *Self, req: zap.Request) !void {
    req.parseQuery();
    ...
    if (try dict_service.getDictLabel(dict_type, dict_value)) |label| {
...
/// 验证字典值是否存在
pub fn validateDictValue(self: *Self, req: zap.Request) !void {
    req.parseQuery();
    ...
    const exists = try dict_service.validateDictValue(dict_type, dict_value);
    
    base.send_ok(req, .{ .exists = exists });
}

/// 刷新字典缓存
pub fn refreshCache(self: *Self, req: zap.Request) !void {
    const service_manager = global.getServiceManager();
    const dict_service = service_manager.getDictService();
    
    try dict_service.refreshDictCache();
    
    base.send_ok(req, .{ .message = "字典缓存已刷新" });
}

/// 获取缓存状态
pub fn getCacheStats(self: *Self, req: zap.Request) !void {
    const service_manager = global.getServiceManager();
    const stats = try service_manager.getCacheStats();
    
    base.send_ok(req, .{ 
        .cache_items = stats.count,
...
/// 清理过期缓存
pub fn cleanupCache(self: *Self, req: zap.Request) !void {
    const service_manager = global.getServiceManager();
